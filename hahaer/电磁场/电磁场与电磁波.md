...
令 S 就是位于平面 z=0 的区域 D（把 D 视为嵌入 R3 的平面片），取单位法向量 n=k=(0,0,1)。其边界 ∂S=∂D 采用**逆时针**方向（与“法向量朝上”的右手定则一致）。
...
$$
这就是格林公式。  
因此，**格林公式是斯托克斯公式在“平面曲面 z=0、向量场 (P,Q,0)”这个特殊情形下的直接推论**。
...
- 电导 G的定义是$G = \frac{1}{R}$，单位就是 **西门子（S）**。
...
- 对于体：有向元素就是体积元 dV，但体积元本身没有“方向”可言（它是个标量体积），所以无法再投影向量场。
...
- $\mathbf{J}$：电流密度，单位是$\text{A/m}^2$，表示单位面积上的电流大小。
    
- $\mathbf{E}$：电场强度，单位是 $\text{V/m}$。
